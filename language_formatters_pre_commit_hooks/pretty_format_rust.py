...
def pretty_format_rust(argv: typing.Optional[typing.List[str]] = None) -> int:
    ...
    for manifest_root in args.manifest_root:
        ...
        filenames = list(
            filter(lambda filename: filename.startswith(manifest_root), args.filenames)
        )
        pretty_format_rust_internal(
            args.autofix,
            manifest_path,
...
def pretty_format_rust_internal(
    autofix: bool,
    manifest_path: str | None,
    filenames: list[str],
    *,
    print_command_exec: bool,
):
    # Check
    if autofix:
        check_args = []
    ...
    status_code, output, _ = run_command(
        ...
        *filenames,
        print_if_ok=False,
        print_command_exec=print_command_exec,
    )
    ...
    if not_well_formatted_files:
        print(
            "{}: {}".format(
                "The following files have been fixed by rustfmt"
                if autofix
