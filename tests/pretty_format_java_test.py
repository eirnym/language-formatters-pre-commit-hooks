...
from language_formatters_pre_commit_hooks.pretty_format_java import (
    _download_palantir_java_formatter_jar,
)
...
@pytest.mark.skipif(
    condition=get_jdk_version() >= Version("21"),
    reason="Skipping test because it requires Java JDK before 21",
)
@pytest.mark.parametrize(
    ("cli_args", "expected_retval"),
    (
...
def test_google_pretty_format_java_up_to_1_9_is_not_allowed_on_jdk_16_and_above(
    undecorate_method,
):
    with pytest.raises(
        ToolNotInstalled,
...
def test_google_pretty_format_java_up_to_1_9_is_allowed_on_jdk_before_16(
    undecorate_method,
):
...
@pytest.mark.skipif(
    True, reason="No publication of old palantir versions has been requested"
)
@pytest.mark.skipif(
    condition=get_jdk_version() <= Version("11"),
    reason="Skipping test because it requires Java JDK above 11",
)
def test_palantir_pretty_format_java_up_to_1_9_is_allowed_on_jdk_11_and_above(
    undecorate_method,
):
    undecorate_method(
        ["--palantir", "--palantir-java-formatter-version=1.9", "pretty-formatted.java"]
    )
...
def test_google_pretty_format_java_jar(
...
def test_palantir_pretty_format_java_jar(
    mock_run_command, undecorate_method, cli_arg, expected_retval
):
    mock_run_command.return_value = (0, "", "")
    assert (
        undecorate_method(["--palantir", cli_arg, "pretty-formatted.java"])
        == expected_retval
    )
    in_args = cli_arg in mock_run_command.call_args.args
    if cli_arg == "":
